...
import com.alibaba.idst.nls.realtime.utils.VoiceCutter;
import com.aliyun.nls.realtime.transcription.opu.OpuCodec;

public class NlsFuture {
	static Logger logger = LoggerFactory.getLogger(NlsFuture.class);
	...
	public NlsFuture sendVoice(byte[] data, int offset, int length) throws Exception {
		if (this.enableOpu) {
			...
			logger.debug("put data in regular mode，use no compress");
			doSend();
		}
	...
	private void doSend() throws InterruptedException, IOException {
		...
		} else if (this.asrSC.toLowerCase().equalsIgnoreCase("pcm")) {
	...
	public class PcmQueue {
		...
		public void consume() throws InterruptedIOException {
			if (NlsFuture.this.asrSC.toLowerCase().equalsIgnoreCase("opu")) {
				...
				if ((voice != null) && (voice.length > 0)) {
					NlsFuture.logger.info("send compressed voice package length: {}", Integer.valueOf(voice.length));
